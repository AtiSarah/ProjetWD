...
<section class="home-section">
    ...
    <div class="set-driver">
        ...
        if (isset($_POST['selected_vehicle'])) {
            ...
            $sql_vehicle = "SELECT license_type FROM vehicle WHERE id_vehicle = ?";
            $stmt_vehicle = $link->prepare($sql_vehicle);
            $stmt_vehicle->bind_param("i", $id_vehicle);
            ...
            if (isset($vehicle_license_type)) {
                ...
                $stmt->bind_param("s", $vehicle_license_type);
                $stmt->execute();
                ...
                if ($result->num_rows > 0) {
                    ...
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<td>
                        <form action='' method='post' onclick='return confirmMission()'>
            ...
            if (isset($_POST['confirm_mission'])) {
                ...
                $sql = "INSERT INTO mission (id_driver, id_vehicle, departure_city, arrival_city, departure_date, duration, cost, type) 
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                $stmt = $link->prepare($sql);
                $stmt->bind_param("iisssids", $id_driver, $id_vehicle, $departureCity, $arrivalCity, $departureDate, $duration, $cost, $type);
                if ($stmt->execute()) {
                    echo '<script>alert("Mission successfully added.");</script>';
...
function confirmMission() {
    return confirm("Are you sure you want to send this mission?");
}
